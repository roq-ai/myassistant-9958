...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiUserCheck, FiHome } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'administrator', 'facility-manager', 'staff-member'];
  ...
  const { data, error, isLoading } = useSWR<ClientInterface[]>(
    ...
    () => getClients({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    { name: 'Facility', path: '/facilities', entity: 'facility', service: AccessServiceEnum.PROJECT, icon: FiHome },
    { name: 'Visitor', path: '/visitors', entity: 'visitor', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
