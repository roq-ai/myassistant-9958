...
import { deleteFacilityById } from 'apiSdk/facilities';
...
function ClientViewPage() {
  ...
  const facilityHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('facility', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/facilities/create?client_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.facility?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/facilities/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess('facility', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/facilities/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('facility', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    facilityHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
